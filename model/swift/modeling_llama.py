...
class LlamaAttention(_LlamaAttention):
    ...
    def forward(
        ...
        if enabled_draft and getattr(self, "collect_draft_attn_scores", False):
            with torch.no_grad():
                draft_scores = attn_weights.detach().mean(dim=(0, 1, 2)).to(torch.float32)
                score_sum = getattr(self, "draft_attn_score_sum", None)
                if score_sum is None or score_sum.numel() < draft_scores.numel():
                    next_score_sum = torch.zeros(
                        draft_scores.numel(),
                        dtype=torch.float32,
                        device=draft_scores.device,
                    )
                    if score_sum is not None:
                        next_score_sum[: score_sum.numel()].copy_(score_sum.to(draft_scores.device))
