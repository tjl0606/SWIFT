...
MODEL_PATH=/home/tjlin/models/Llama-2-7b-hf
...
CUDA_VISIBLE_DEVICES=${GPU_DEVICES} python -m evaluation_llama.inference_swift --model-path $MODEL_PATH --model-id ${MODEL_NAME} \
  ...
  --max-tolerance-iter ${MAX_TOLERANCE_ITER} --max-score ${MAX_SCORE} --skip-ratio ${SKIP_RATIO} --bayes --draft-kv-compress --draft-kv-retain-ratio 1.0 --draft-only #--cache-hit --optimization 

#Mean accepted tokens: 1.6295661557015455
# Token acceptance rate: 0.6162089960238568

#Mean accepted tokens: 1.5854253512428593
# Token acceptance rate: 0.579254506568897