...
TASK_NAME="mmlu" # cnndm, humaneval
DATA_NUM=100
MAX_NEW_TOKENS=256
TOP_P=1.0
...
CUDA_VISIBLE_DEVICES=${GPU_DEVICES} python -m evaluation_llama.inference_swift --model-path $MODEL_PATH --model-id ${MODEL_NAME} \
  ...
  --max-tolerance-iter ${MAX_TOLERANCE_ITER} --max-score ${MAX_SCORE} --skip-ratio ${SKIP_RATIO}  --optimization --bayes --draft-kv-compress --draft-kv-retain-ratio 0.9  #--cache-hit  --draft-only 
